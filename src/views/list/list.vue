<template>
  <div v-loading.fullscreen.lock="loading" class="naire-list">
    ...
    <el-table :data="list" @selection-change="onSelectionChange">
      ...
      <el-table-column prop="nintro" label="调查简介" align="left">
        <template slot-scope="{ row }">
          {{ row.nintro }}
        </template>
      </el-table-column>
      <el-table-column prop="ncreattime" label="创建时间" align="center">
        <template slot-scope="{ row }">
          {{ row.ncreattime }}
        </template>
      </el-table-column>
      <el-table-column prop="ndeadline" label="截止时间" align="center">
        <template slot-scope="{ row }">
          <!-- 问卷超过截止日期 -->
          {{ row.ndeadline }}
        </template>
      </el-table-column>
      <el-table-column prop="nstatus" label="发布状态" align="center">
        <template slot-scope="{ row }">
          <el-tag :type="row.nstatus | statusColorFilter">
            {{ row.nstatus | statusFilter }}
          </el-tag>
        </template>
      </el-table-column>
